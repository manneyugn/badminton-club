    ...
    <div ref={ref} className="relative w-full">
      <div
        ...
        <div className="flex items-center gap-2">
          {selected && (
            <span className="text-xs px-1 rounded" style={{ color: 'var(--muted)' }}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(p => (
                <div key={p.player_id}
                  ...
                  style={{ background: p.player_id === value ? '#14532d' : 'transparent', color: p.player_id === value ? 'var(--accent)' : 'var(--foreground)' }}
                  ...
                  <span style={{ color: p.player_id === value ? 'var(--accent)' : 'var(--muted)' }}>{Math.round(p.elo)}</span>
                </div>
              ))
            )}
          </div>
